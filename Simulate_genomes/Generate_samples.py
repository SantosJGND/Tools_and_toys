...
def Gen_samples_II(Pops,Sizes,vector_lib,label_package,Origins,prior_func,prior_kwargs,Cop_choice,window_size= 5000,Chr= 1,return_pca= False,n_comp= 100,range_diff= [-10,10],steps= 100):
    ...
    for angle in np.linspace(range_diff[0],range_diff[1],steps):
        ...
        coords= prior_func(coords,angle,**prior_kwargs)
...
def Check_Path(Npops,vector_lib,prior_func,prior_kwargs,Pops= [], random= True,n_comp= 100,range_diff= [-10,10],steps= 100):
    ...
    for angle in np.linspace(range_diff[0],range_diff[1],steps):
        ...
        coords, prior= prior_func(coords,angle,passport= True,**prior_kwargs)
