...
def Euc_to_fst(vector_lib,n_comp= 5,pop_max= 8,Iter= 20,bias_range= [20,300],Eigen= False, Scale= False,Centre= True,ploidy= 1,fst_ranfer= [0.01,.3]):
...
def Fst_predict(vector_lib,m_coeff,b,n_comp= 5,pop_max= 8,Iter= 20,bias_range= [20,300],Eigen= False, Scale= False,Centre= True,ploidy= 1):
    ...
    for rep in range(Iter):
        ...
        for k in range(N_pops):
            ...
            m= bias_scheme[k]
            Haps= [[np.random.choice([0,ploidy],p= [1-probs[x],probs[x]]) for x in range(length_haps)] for acc in range(m)]
