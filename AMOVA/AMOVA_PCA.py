...
for CHR in Construct.keys():
    for bl in Construct[CHR].keys():
        for hill in Construct[CHR][bl].keys():
            Clover.append(Construct[CHR][bl][hill])
...
##
filename= Home + args.id + '_KDE_pca_var.txt'
os.makedirs(os.path.dirname(filename), exist_ok=True)
Output= open(filename,'w')

Output.write('\t'.join(['pc' + str(x + 1) for x in range(X_se.shape[1])]))
Output.write('\n')

Output.write('\t'.join([str(x) for x in COMPS81]))
Output.close()

##
filename= Home + args.id + '_KDE_pca.txt'
os.makedirs(os.path.dirname(filename), exist_ok=True)
Output= open(filename,'w')

Output.write('id\t' + '\t'.join(['pc' + str(x + 1) for x in range(X_se.shape[1])]))
Output.write('\n')

for ind in range(X_se.shape[0]):
    name= Whose[ind]
    name= Fam[name]
    Output.write(name + '\t' + '\t'.join([str(x) for x in X_se[ind,:]]))
    Output.write('\n')
